...
test("claim service creates claim and explicit version 1 in one transaction", async () => {
  ...
  assert.match(harness.calls[2].text, /based_on_version_id/);
  ...
  assert.equal(versionValues[10], null);
  assert.equal(versionValues[11], "api");
  assert.equal(versionValues[12], null);
  assert.equal(versionValues[13], "api");
  assert.equal(versionValues[14], null);
  assert.match(versionValues[15], /^[0-9a-f-]{36}$/i);
